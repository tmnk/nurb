...
verb_eval_Tess.rationalCurveAdaptiveSample = function(curve,tol,includeU) {
	...
	if(curve.degree == 1) {
	 ...
	 verb_eval_Tess.rationalCurveAdaptiveSampleRange(curve,curve.knots[0],verb_core_ArrayExtensions.last(curve.knots),tol,includeU);
	 return hoo;
};
verb_eval_Tess.rationalCurveAdaptiveSampleRange = function(curve,start,end,tol,includeU) {
	...
	if (!tmpgiga) {
		for (var i = 0, j = 0; i <= 1; i += 0.05, j++ ) hoo[j] = verb_eval_Eval.rationalCurvePoint(curve,i);
			tmpgiga  = 1;
	}
